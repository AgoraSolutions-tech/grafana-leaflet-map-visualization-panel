import React, { useState } from 'react';
import { Button, FieldArray, HorizontalGroup, Input, Label } from '@grafana/ui';

interface Props {
  control: any;
  register: any;
  index: number;
  initialIsOpen?: boolean;
}

export const VerticlesForm = ({ control, register, index, initialIsOpen = false }: Props) => {
  const [isOpen, setIsOpen] = useState(initialIsOpen);

  const verticlesButtonText = isOpen ? 'Verticles:': 'Show verticles';

  return (
    <>
    <Button 
      variant="secondary" 
      size="xs" 
      fill={isOpen ? 'text' : 'outline'}
      onClick={() => setIsOpen(!isOpen)}
      icon={isOpen ? 'angle-up' : 'angle-down'}
      tooltip={isOpen ? 'Hide verticles' : ''}
    >
      {verticlesButtonText}
    </Button>
    <div style={{display: isOpen ? 'block' : 'none' }}>
      <FieldArray control={control} name={`areas.${index}.verticles`}>
        {({ append, fields, remove }) => (
          <>
            <div style={{ marginBottom: '1rem' }}>
              {fields.map((vertex, vertexIndex) => (
                <div key={vertex.id}>
                  <div key={vertexIndex}>
                    <input type="hidden" {...register(`areas.${index}.verticles.${vertexIndex}.id` as const)} />
                    <HorizontalGroup>
                      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                        <Label>{vertexIndex + 1}</Label>
                      </div>
                      <div>
                        {vertexIndex === 0 && <Label>Latitude: </Label>}
                        <Input
                          {...register(`areas.${index}.verticles.${vertexIndex}.lat` as const, {
                            valueAsNumber: true,
                          })}
                          defaultValue={vertex.lat}
                        />
                      </div>
                      <div>
                        {vertexIndex === 0 && <Label>Longitude: </Label>}
                        <Input
                          {...register(`areas.${index}.verticles.${vertexIndex}.lng` as const, {
                            valueAsNumber: true,
                          })}
                          defaultValue={vertex.lng}
                        />
                      </div>
                      <Button
                        icon="trash-alt"
                        onClick={() => {
                          if (fields.length >= 4) {
                            remove(vertexIndex);
                          }
                        }}
                        variant="secondary"
                        size="sm"
                        fill="text"
                      />
                    </HorizontalGroup>
                  </div>
                </div>
              ))}
            </div>
            <Button
              style={{ marginRight: '1rem' }}
              onClick={() =>
                append({
                  id: Math.random(),
                  lat: fields[fields.length - 1].lat,
                  lng: fields[fields.length - 1].lng,
                })
              }
              variant="secondary"
              size="sm"
              icon="plus"
            >
              Add vertex
            </Button>
          </>
        )}
      </FieldArray>
      </div>
    </>
  );
};
